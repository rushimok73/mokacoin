...
contract('MokaToken', function(accounts) {
  ...
  it('approve transactions of tokens', function(){
    return MokaToken.deployed().then(function(instance){
      ...
      assert.equal(receipt.logs[0].args._owner, accounts[0], 'logs the account of the owner');
      ...
      assert.equal(allowance.toNumber(), 100, 'it sets the allowance correctly in mapping');
    });
  });
  ...
  it('handles delegated token transfers', function() {
      ...
      assert.equal(allowance.toNumber() , 0, 'deducts the allowance properly');
    });
  });
